...
export default function FashionBeautyBookMeetingPage() {
  ...
  useEffect(() => {
    const fetchBookingData = async () => {
      try {
        ...
        console.error('Failed to load fashion beauty booking data', error);
  ...
  const handleRequestBooking = async () => {
    ...
    try {
      ...
      setBookingNote('');
      setToast({ message: 'Booking request sent. The beauty vendor will review and approve it.', type: 'success' });
    } catch (error) {
      console.error('Failed to create fashion beauty booking request', error);
      setToast({
        ...
        <div className="lg:col-span-2 space-y-6">
          <section className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="relative h-72 md:h-96 bg-gray-100 overflow-hidden">
              <img src={packageData.images?.[0] || '/saloon.png'} alt={packageData.name} className="w-full h-full object-cover" />
              {packageData.discount && (
                <div className="absolute top-6 right-6 bg-red-500 text-white px-6 py-3 rounded-full text-lg font-bold shadow-lg">{packageData.discount} OFF</div>
              )}
            </div>
        ...
        <aside className="space-y-6">
          <section className="bg-white rounded-xl shadow-md p-6 sticky top-24">
            ...
            <div className="mb-6">
              ...
              <textarea value={bookingNote} onChange={(e) => setBookingNote(e.target.value)} rows={4} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900 placeholder-gray-400" placeholder="Tell the beauty vendor what you'd like to discuss..." />
            </div>
