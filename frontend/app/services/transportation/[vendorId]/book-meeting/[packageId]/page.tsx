...
import { FaArrowLeft, FaCheckCircle, FaClock, FaHome, FaPaperPlane, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
const categoryLabels: Record<string, string> = {
  ...
  'bridal-cars': 'Wedding Cars',
  'luxury-vehicles': 'Luxury Vehicles',
  'guest-transport': 'Guest Transport',
};

export default function TransportationBookMeetingPage() {
  ...
  useEffect(() => {
    const fetchBookingData = async () => {
      try {
        ...
        console.error('Failed to load transportation booking data', error);
        setToast({ message: 'Unable to load booking calendar. Please try again.', type: 'error' });
      } finally {
        setLoading(false);
      }
    };

    if (vendorId && packageId) fetchBookingData();
  }, [packageId, vendorId]);
  ...
  const handleRequestBooking = async () => {
    ...
    try {
      ...
      setBookingNote('');
      setToast({ message: 'Booking request sent. The transportation vendor will review and approve it.', type: 'success' });
    } catch (error) {
      console.error('Failed to create transportation booking request', error);
      setToast({ message: `Unable to create booking request: ${error instanceof Error ? error.message : 'Unknown error'}`, type: 'error' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading meeting calendar...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        ...
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="px-6 py-3 rounded-lg text-white" style={{ backgroundColor: '#755A7B' }}>Go Back</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-20 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <Link href="/" className="inline-flex items-center gap-2 rounded-full border border-emerald-500 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50">
            <FaHome /> Home
          </Link>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <section className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="relative h-72 md:h-96 bg-gray-100 overflow-hidden">
              <img src={packageData.images?.[0] || '/car.png'} alt={packageData.name} className="w-full h-full object-cover" />
              {packageData.discount && <div className="absolute top-6 right-6 bg-red-500 text-white px-6 py-3 rounded-full text-lg font-bold shadow-lg">{packageData.discount} OFF</div>}
            </div>
            <div className="p-6 md:p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500 mb-2">{categoryLabels[packageData.category] || packageData.category}</p>
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3">{packageData.name}</h2>
              <p className="text-gray-600 leading-relaxed whitespace-pre-wrap mb-6">{packageData.description || 'No description provided.'}</p>
          ...
          <section className="bg-white rounded-xl shadow-md p-6 md:p-8">
            <div className="flex items-center justify-between gap-4 mb-4">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"><FaChevronLeft /></button>
                <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"><FaChevronRight /></button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 gap-2">
              {Array.from({ length: startingDayOfWeek }).map((_, index) => <div key={`empty-${index}`} className="h-16 md:h-20" />)}
              {calendarDays.map(({ day, dateStr }) => {
                const booked = hasBookingOnDate(dateStr);
                const selected = selectedDate === dateStr;
                return (
                  <button key={dateStr} onClick={() => setSelectedDate(dateStr)} className={`h-16 md:h-20 rounded-xl border-2 p-2 text-left transition-all ${selected ? 'shadow-lg' : ''}`} style={{
                    ...
                    <div className="flex h-full flex-col justify-between">
                      ...
                      <span className={`text-[10px] font-semibold ${booked ? 'text-red-500' : 'text-green-600'}`}>{booked ? 'Booked' : 'Open'}</span>
                    </div>
                  </button>
                );
              })}
        ...
        <aside className="space-y-6">
          <section className="bg-white rounded-xl shadow-md p-6 sticky top-24">
            <div className="mb-6">
              ...
              <h3 className="text-2xl font-bold mb-2" style={{ color: '#755A7B' }}>Rs. {Number(packageData.price || 0).toLocaleString()}</h3>
              <p className="text-gray-500 text-sm">package price</p>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {timeSlots.map((slot) => {
                  ...
                  return (
                    <button key={slot} type="button" disabled={booked} onClick={() => setSelectedTime(slot)} className="rounded-lg px-3 py-2 text-sm font-medium transition-all disabled:cursor-not-allowed" style={{
            ...
            <div className="mb-6">
              ...
              <input value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900" placeholder="07X XXX XXXX" />
            </div>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Booking Note</label>
              <textarea value={bookingNote} onChange={(e) => setBookingNote(e.target.value)} rows={4} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900 placeholder-gray-400" placeholder="Tell the transport vendor what you'd like to discuss..." />
            </div>
            <button onClick={handleRequestBooking} disabled={submitting} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold text-white transition-all hover:opacity-90 disabled:opacity-70" style={{ backgroundColor: '#755A7B' }}>
