...
const emptyVendor = (vendorId: string): Vendor => ({
  id: vendorId,
  name: 'Cake Decoration Vendor',
  organizationName: 'Cake Decoration Vendor',
  category: [],
  location: 'Not specified',
  rating: 4.7,
  reviewCount: 0,
  image: '/cake.png',
  about: 'Professional cake decoration packages for weddings and special events.',
  features: [],
});
...
export default function CakeVendorDetail() {
    ...
    try {
      ...
      const livePackages = offerings.flatMap((offering): Package[] => {
        ...
        return [{
          ...
          photos: Array.isArray(offering.images) && offering.images.length ? offering.images : ['/cake.png'],
      ...
      const organizationName = firstOfferingVendor?.organizationName || firstOfferingVendor?.businessName || firstOfferingVendor?.name || 'Cake Decoration Vendor';
      ...
      setVendor({
        ...
        image: livePackages[0]?.photos[0] || '/cake.png',
        about: firstOfferingVendor?.description || livePackages[0]?.description || 'Professional cake decoration packages for weddings and special events.',
        features: Array.from(new Set(livePackages.flatMap((pkg) => pkg.features))),
      });
    } catch (error) {
      console.error('Failed to fetch cake vendor packages:', error);
      setVendor(emptyVendor(vendorId));
      setPackages([]);
      setToast({ message: 'Unable to load cake vendor packages.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const addToBudgetCalculator = (pkg: Package, e: React.MouseEvent) => {
    ...
    budgetPackageDetails.push({ packageId: pkg.id, vendorId: vendor.id, category: pkg.category, title: pkg.title, price, image: pkg.photos[0], vendorName: vendor.organizationName, quantity: 1, href: `/services/cake-decoration/${vendor.id}`, serviceSlug: 'cake-decoration' });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading cake packages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Link href="/services/cake-decoration" className="text-purple-600 hover:text-purple-800 mb-4 inline-block">Back to Cake Decoration Services</Link>
