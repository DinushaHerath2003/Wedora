...
const defaultFeatures = ['Personalized Tags', 'Custom Packaging', 'Thank You Cards', 'Ribbon & Floral Wrap', 'Bulk Order Prep', 'Delivery Service', 'Name Engraving', 'Gift Table Styling'];
...
export default function GiftingSouvenirsServices() {
  ...
  const fetchGiftOfferings = async () => {
    try {
      setLoading(true);
      ...
      const livePackages = offerings.flatMap((offering): DisplayPackage[] => {
        ...
        return [{
          ...
          vendorName: offering.vendor.organizationName || offering.vendor.businessName || offering.vendor.name || 'Gifting Vendor',
          category: normalizedCategory,
          location: offering.vendor.location || 'Not specified',
          title: offering.name,
          description: offering.description,
          price: Number(offering.price) || 0,
          image: Array.isArray(offering.images) && offering.images.length ? offering.images[0] : '/gift.png',
      ...
      console.error('Failed to fetch gifting offerings:', error);
      setPackages([]);
      setToast({ message: 'Unable to load gifting packages.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const addToBudgetCalculator = (pkg: DisplayPackage) => {
    ...
    budgetPackageDetails.push({ packageId: pkg.id, vendorId: pkg.vendorId, category: pkg.category, title: pkg.title, price, image: pkg.image, vendorName: pkg.vendorName, quantity: 1, href: `/services/gifting-souvenirs/${pkg.vendorId}`, serviceSlug: 'gifting-souvenirs' });
  ...
  const handleAddToCart = (pkg: DisplayPackage) => {
    const updatedCart = addCartItem({
      id: `gifting-souvenirs-${pkg.vendorId}-${pkg.id}`,
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading gifting packages...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative h-64 overflow-hidden">
        <img src="/gift.png" alt="Gifting & Souvenirs" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-opacity-50 flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-4xl font-bold mb-4" style={{ fontFamily: 'var(--font-season)' }}>Gifting & Souvenirs</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search by package, gift provider, or location..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
          ...
          <div className="mt-6 flex gap-2 overflow-x-auto pb-2">
            ...
            {(['wedding-favors', 'gift-boxes', 'custom-souvenirs'] as GiftCategory[]).map((category) => (
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <label className="flex-1 text-sm text-gray-600">Max Price
                      <input type="number" value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], parseInt(e.target.value) || 100000])} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    </label>
                  </div>
                </div>
              </div>
              <button onClick={() => { setSelectedCategory('all'); setSearchQuery(''); setSelectedFeatures([]); setPriceRange([0, 100000]); }} className="mt-4 px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">Clear All Filters</button>
            </div>
          )}
        </div>
            ...
            <div key={pkg.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow">
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-1 gap-2 mt-4">
                  <button onClick={() => router.push(`/services/gifting-souvenirs/${pkg.vendorId}`)} className="px-3 py-2 rounded-lg font-medium border-2 transition-all hover:shadow-sm" style={{ borderColor: '#755A7B', color: '#755A7B', backgroundColor: 'white' }}>See More</button>
        ...
        {filteredPackages.length === 0 && (
          <div className="text-center py-16">
            <p className="text-xl text-gray-500 mb-4">No gifting packages found matching your criteria</p>
            <button onClick={() => { setSelectedCategory('all'); setSearchQuery(''); setSelectedFeatures([]); setPriceRange([0, 100000]); }} className="px-6 py-3 rounded-lg font-medium text-white" style={{ backgroundColor: '#755A7B' }}>Clear Filters</button>
          </div>
        )}
      </div>
