...
const venueCategories = ['venue-accommodation'];
...
export default function VenueSettingsPage() {
  ...
  const organizationLabel = form.organizationName || user?.organizationName || user?.name || 'Venue Vendor';
    ...
    const loadProfile = async () => {
      try {
        ...
        setForm({
          ...
          categories: data.categories?.length ? data.categories : venueCategories,
        });
      } catch (error) {
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="mb-6">
            ...
            <button onClick={() => router.push('/dashboard/venue-accommodation/overview')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaChartBar /> Overview</button>
            <button onClick={() => router.push('/dashboard/venue-accommodation')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaPlus /> Post Package</button>
            <button onClick={() => router.push('/dashboard/venue-accommodation/posted-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaFileInvoice /> Posted Packages</button>
            <button onClick={() => router.push('/dashboard/venue-accommodation/draft-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaEdit /> Draft Package</button>
          </div>

          <div className="mb-6">
            <p className="text-xs font-semibold text-gray-400 mb-2 px-3">Appointment</p>
            <button onClick={() => router.push('/dashboard/venue-accommodation/place-booking')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 text-gray-600 hover:bg-gray-100"><FaCalendarAlt /> Place a Booking</button>
            <button onClick={() => router.push('/dashboard/venue-accommodation/accept-booking')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 text-gray-600 hover:bg-gray-100"><FaEye /> Accept Booking</button>
          </div>
