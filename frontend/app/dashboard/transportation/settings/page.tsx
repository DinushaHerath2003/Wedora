...
const transportationCategories = ['transportation'];
...
export default function TransportationSettingsPage() {
  ...
  const organizationLabel = form.organizationName || user?.organizationName || user?.name || 'Transportation Vendor';
    ...
    const loadProfile = async () => {
      try {
        ...
        setForm({
          ...
          categories: data.categories?.length ? data.categories : transportationCategories,
        });
      } catch (error) {
        console.error('Failed to load transportation vendor profile', error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Failed to save transportation vendor profile', error);
      setToast({
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <section className="xl:col-span-1 bg-white rounded-xl shadow-md p-6">
            ...
            <div className="-mt-14 mb-5 flex justify-center">
              <div className="w-20 h-20 rounded-full border-4 border-white flex items-center justify-center text-white text-3xl font-bold shadow-lg" style={{ backgroundColor: '#755A7B' }}>{organizationInitial}</div>
            </div>
          ...
          <section className="xl:col-span-2 bg-white rounded-xl shadow-md p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <label className="block">
                ...
                <input value={form.organizationName} onChange={(e) => setForm({ ...form, organizationName: e.target.value })} className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-900 font-medium placeholder-gray-400" />
              </label>
              <label className="block">
              ...
              <label className="block">
                ...
                <input value={form.contactPerson} onChange={(e) => setForm({ ...form, contactPerson: e.target.value })} className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-900 font-medium placeholder-gray-400" />
              </label>
              <label className="block">
                <span className="block text-sm font-medium text-gray-700 mb-2">Phone Number</span>
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-900 font-medium placeholder-gray-400" />
              </label>
              <label className="block md:col-span-2">
                <span className="block text-sm font-medium text-gray-700 mb-2">Business Location</span>
                <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-900 font-medium placeholder-gray-400" />
              </label>
            </div>
            ...
            <div className="mt-6 rounded-xl border border-purple-100 p-4" style={{ backgroundColor: 'rgba(117, 90, 123, 0.04)' }}>
              ...
              <span className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-white" style={{ backgroundColor: '#755A7B' }}>Transportation</span>
            </div>
          </section>
        </div>
      </main>
