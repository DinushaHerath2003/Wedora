...
export default function TransportationPackageDetailPage() {
  ...
  useEffect(() => {
    const loadPackage = async () => {
      try {
        ...
        console.error('Failed to load transportation package details', error);
        setToast({
  ...
  const images = packageData.images?.length ? packageData.images : ['/car.png'];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="relative h-72 md:h-96 bg-gray-100">
                ...
                {packageData.discount && <div className="absolute top-6 right-6 bg-red-500 text-white px-5 py-2 rounded-full text-sm font-bold shadow-lg">{packageData.discount} OFF</div>}
              </div>
              {images.length > 1 && (
                <div className="p-6 border-t">
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-6 gap-3">
                    {images.map((image, index) => (
                      <button key={image} onClick={() => setSelectedImageIndex(index)} className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${selectedImageIndex === index ? 'border-purple-600 shadow-lg' : 'border-gray-200 hover:border-purple-400'}`}>
              ...
              {!!packageData.facilities?.length && (
                <div className="mt-8 pt-8 border-t">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {packageData.facilities.map((feature) => (
                      <div key={feature} className="flex items-center gap-3 p-3 rounded-lg" style={{ backgroundColor: 'rgba(117, 90, 123, 0.05)' }}>
          ...
          <aside className="lg:col-span-1 space-y-6">
            <section className="bg-white rounded-xl shadow-md p-6 sticky top-24">
              ...
              <h2 className="text-3xl font-bold mb-2" style={{ color: '#755A7B' }}>Rs. {Number(packageData.price || 0).toLocaleString()}</h2>
              ...
              <button onClick={() => router.push(`/services/transportation/${packageData.vendorId}/book-meeting/${packageData.id}`)} className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold text-white transition-all hover:opacity-90" style={{ backgroundColor: '#10b981' }}>
            ...
            <section className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex gap-1">
                  {[...Array(5)].map((_, index) => <FaStar key={index} size={18} style={{ color: index < Math.round(averageRating) ? '#fbbf24' : '#d1d5db' }} />)}
                </div>
