...
export default function TransportationAcceptBookingPage() {
  ...
  const filteredBookings = useMemo(
    () => bookings.filter((booking) => normalizeCategory(booking.offering?.category) === activeCategory),
  ...
  const updateBookingStatus = async (id: number, status: BookingStatus) => {
    try {
      ...
      console.error('Failed to update transportation booking', error);
  ...
  return (
    <div className="flex min-h-screen flex-col md:flex-row" style={{ backgroundColor: '#f5f5f7' }}>
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="mb-6">
            ...
            <button onClick={() => router.push('/dashboard/transportation/draft-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaEdit /> Draft Package</button>
          </div>
          <div className="mb-6">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors" style={{ backgroundColor: '#755A7B', color: 'white' }}><FaEye /> Accept Booking</button>
          </div>
          <div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        <div className="mb-6 md:mb-8 rounded-lg overflow-hidden shadow-lg" style={{ backgroundImage: 'url(/car.png)', backgroundSize: 'cover', backgroundPosition: 'center', height: '180px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: '2px solid rgba(117, 90, 123, 0.2)' }}>
          ...
          <p className="text-sm md:text-xl text-white" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.7)' }}>Review customer transportation requests and approve available slots</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-md">
            <h3 className="text-xl font-semibold text-gray-700">No booking requests found</h3>
            <p className="text-gray-500 mt-2">Requests for {categoryLabel(activeCategory).toLowerCase()} will appear here.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2 space-y-4">
              {filteredBookings.map((booking) => (
                <article key={booking.id} className="bg-white rounded-xl shadow-md p-5 md:p-6 hover:shadow-lg transition-shadow">
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                    <div className="rounded-lg p-4" style={{ backgroundColor: 'rgba(117, 90, 123, 0.05)' }}>
                      ...
                      <p className="font-semibold text-gray-800">{booking.offering?.name || 'Transportation package'}</p>
                    </div>
                    <div className="flex gap-2 justify-start md:justify-end">
                      <button onClick={() => updateBookingStatus(booking.id, 'confirmed')} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 font-semibold text-white"><FaCheck /> Approve</button>
                      <button onClick={() => updateBookingStatus(booking.id, 'cancelled')} className="inline-flex items-center gap-2 rounded-lg border border-red-500 px-4 py-2 font-semibold text-red-600"><FaTimes /> Reject</button>
                    </div>
                  </div>
                </article>
              ))}
            ...
            <aside className="bg-white rounded-xl shadow-md p-6 h-fit">
              <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">Summary</p>
              <h2 className="text-3xl font-bold text-gray-900">{filteredBookings.length}</h2>
              <p className="text-gray-500">booking request{filteredBookings.length === 1 ? '' : 's'} in {categoryLabel(activeCategory)}</p>
            </aside>
          </div>
        )}
      </main>
