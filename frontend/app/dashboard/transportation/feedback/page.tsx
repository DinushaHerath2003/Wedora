...
export default function TransportationFeedbackPage() {
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      try {
        ...
        console.error('Failed to load transportation feedback', error);
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
          <section className="bg-white rounded-xl shadow-md p-6">
            ...
            <div className="flex gap-1 mb-2">
              {[...Array(5)].map((_, index) => <FaStar key={index} style={{ color: index < Math.round(averageRating) ? '#fbbf24' : '#d1d5db' }} />)}
            </div>
        ...
        {loading ? (
            ...
            {Object.entries(groupedByPackage).map(([packageName, packageReviews]) => (
              <section key={packageName} className="bg-white rounded-xl shadow-md p-6">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5">
                  ...
                  <button onClick={() => router.push(`/dashboard/transportation/posted-packages/${packageReviews[0].offeringId}`)} className="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 font-medium text-white" style={{ backgroundColor: '#755A7B' }}>
                    <FaEye /> View Package
                  </button>
                </div>
                    ...
                    <article key={review.id} className="rounded-xl border border-gray-200 p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <div className="flex gap-1">
                          {[...Array(5)].map((_, index) => <FaStar key={index} size={15} style={{ color: index < review.rating ? '#fbbf24' : '#d1d5db' }} />)}
                        </div>
                      </div>
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-md">
            ...
            <p className="text-gray-500 mt-2">Customer feedback for your transportation packages will appear here.</p>
          </div>
        )}
      </main>
    </div>
