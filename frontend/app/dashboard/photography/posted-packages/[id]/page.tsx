...
export default function PhotographyPackageDetailPage() {
  ...
  useEffect(() => {
    const loadPackage = async () => {
      try {
        ...
        console.error('Failed to load photography package details', error);
        setToast({
  ...
  const images = packageData.images?.length ? packageData.images : ['/photography.png'];
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Photography & videography</p>
            </div>
          </div>
          <button
            onClick={() => router.push('/')}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="lg:col-span-1 space-y-6">
            <section className="bg-white rounded-xl shadow-md p-6 sticky top-24">
              ...
              <button
                onClick={() => router.push(`/services/photography-videography/${packageData.vendorId}/book-meeting/${packageData.id}`)}
