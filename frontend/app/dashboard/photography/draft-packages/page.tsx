...
export default function DraftPackagesPage() {
  ...
  const getCategoryBannerText = () => {
    switch (activeCategory) {
      case 'wedding-photography':
        return 'Wedding Photography Drafts';
      case 'pre-wedding-shoots':
        return 'Pre-Wedding Shoots Drafts';
      case 'videography':
        return 'Videography Drafts';
      default:
      ...
      setPackages(
        offerings
          ...
          .map((offering) => {
            ...
            if (category === 'wedding-photography' || category === 'wedding-photography') {
              mappedCategory = 'wedding-photography';
            } else if (category === 'pre-wedding-shoots' || category === 'pre-wedding-shoots') {
              mappedCategory = 'pre-wedding-shoots';
            } else if (category === 'videography' || category === 'videography') {
    ...
    try {
      ...
      await apiFetch(`/offerings/${id}`, {
        ...
        body: JSON.stringify({
          ...
          description: currentPackage.features.join(' '),
          category: currentPackage.category,
          price: currentPackage.pricePerDay,
          facilities: currentPackage.features,
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          ...
          <div>
            ...
            <button onClick={() => router.push('/dashboard/photography/feedback')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 text-gray-600 hover:bg-gray-100"><FaHeart /> Feedback</button>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
              ...
              <div key={pkg.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group">
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={() => router.push(`/services/photography-videography/${user?.id || ''}`)} className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-emerald-500 font-semibold text-emerald-700 bg-white transition-all hover:bg-emerald-600 hover:text-white">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => router.push(`/dashboard/photography?edit=${pkg.id}`)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg border-2 font-medium transition-all" style={{ borderColor: '#755A7B', color: '#755A7B', backgroundColor: 'white' }}>
                        <FaEdit size={16} /> Edit
                      </button>
                    </div>
          ...
          {filteredPackages.length === 0 && (
            <div className="text-center py-16">
              <FaEdit className="mx-auto text-6xl text-gray-300 mb-4" />
