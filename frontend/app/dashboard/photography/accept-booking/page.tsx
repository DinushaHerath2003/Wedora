...
export default function AcceptBookingPage() {
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      const category = booking.offering?.category || 'wedding-photography';
      const normalized = category === 'wedding-photography' || category === 'wedding-photography'
        ? 'wedding-photography'
        : category === 'pre-wedding-shoots' || category === 'pre-wedding-shoots'
        ? 'pre-wedding-shoots'
        : 'videography';
    ...
    <div className="flex min-h-screen flex-col md:flex-row" style={{ backgroundColor: '#f5f5f7' }}>
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="mb-6">
            ...
            <button onClick={() => router.push('/dashboard/photography/overview')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100">
              <FaChartBar /> Overview
            </button>
            ...
            <button onClick={() => router.push('/dashboard/photography/draft-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100">
      ...
      <div className="flex-1 flex flex-col">
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          ...
          <div className="mb-6 md:mb-8 rounded-lg overflow-hidden shadow-lg" style={{ backgroundImage: 'url(/photography.png)', backgroundSize: 'cover', backgroundPosition: 'center', height: '180px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: '2px solid rgba(117, 90, 123, 0.2)' }}>
          ...
          <div className="mb-6">
            <div className="flex gap-2 md:gap-6 justify-center items-center overflow-x-auto pb-2">
              {['wedding-photography', 'pre-wedding-shoots', 'videography'].map((category) => (
                <button
                  key={category}
                  onClick={() => setActiveCategory(category as PhotoCategory)}
                ...
                >
                  {category === 'wedding-photography' ? 'Wedding Photography' : category === 'pre-wedding-shoots' ? 'Pre-Wedding Shoots' : 'Videography'}
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="xl:col-span-2 space-y-4">
                  ...
                  <div key={booking.id} className="bg-white rounded-xl shadow-md p-5 md:p-6 hover:shadow-lg transition-shadow">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                      <div className="rounded-lg p-4" style={{ backgroundColor: 'rgba(117, 90, 123, 0.05)' }}>
                        ...
                        <p className="font-semibold text-gray-800">{booking.offering?.name || 'Photography package'}</p>
                        <p className="text-sm text-gray-600">{booking.offering?.category || 'photography package'}</p>
                      </div>
                ...
                {filteredBookings.length === 0 && (
                  <div className="bg-white rounded-xl shadow-md p-10 text-center">
                    ...
                    <p className="text-gray-500">Requests for the selected photography category will appear here.</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => router.push('/dashboard/photography/place-booking')} className="w-full rounded-lg px-4 py-3 font-medium text-gray-700 border-2 border-gray-300 hover:border-gray-400 transition-colors">
                      Open Calendar
                    </button>
                    <button onClick={() => router.push('/dashboard/photography/posted-packages')} className="w-full rounded-lg px-4 py-3 font-medium text-white transition-colors" style={{ backgroundColor: '#755A7B' }}>
