...
export default function PhotographyDashboard() {
  ...
  const [packageCategory, setPackageCategory] = useState('wedding-photography');
  ...
  useEffect(() => {
    ...
    const loadPackageForEdit = async () => {
      try {
        ...
        setActiveCategory(
          data.category === 'pre-wedding-shoots'
            ? 'pre-wedding-shoots'
            : data.category === 'videography'
            ? 'videography'
            : 'wedding-photography'
        );
        setPackageCategory(data.category || 'wedding-photography');
    ...
    try {
      ...
      setPackages(
        offerings.map((offering) => ({
          ...
          category: offering.category as PhotoCategory,
          title: offering.name,
  ...
  const handleSubmitPackage = async (e?: React.FormEvent, isDraft = false) => {
    ...
    try {
      ...
      const packageData: Package = {
        ...
        category: createdPackage.category as PhotoCategory,
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          ...
          <div 
            ...
            style={{
              backgroundImage: 'url(/photography.png)',
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Premium Wedding Photography Package"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={5}
                      placeholder="Professional wedding photography package with high-quality equipment. Includes full-day coverage, edited photos, online gallery, albums, optional drone shots, and cinematic storytelling for the celebration..."
                    />
                    {newPackage.features.trim().split(/\s+/).filter(w => w).length < 200 && newPackage.features.trim().length > 0 && (
                      <p className="text-xs text-orange-600 mt-2">
                        âš ï¸ Description must be at least 200 words. {200 - newPackage.features.trim().split(/\s+/).filter(w => w).length} more words needed.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['HD Photos', 'Album', 'Online Gallery', 'Drone Shots'].map((facility) => (
                          <button
                            key={facility}
                            type="button"
