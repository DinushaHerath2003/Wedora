...
export default function AcceptBookingPage() {
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      const category = normalizeCeremonialCategory(booking.offering?.category);
          ...
          <div className="mb-6">
            <div className="flex gap-2 md:gap-6 justify-center items-center overflow-x-auto pb-2">
              {['poruwa-ceremony', 'religious-services', 'cultural-events'].map((category) => (
                <button
                  key={category}
                  onClick={() => setActiveCategory(category as CeremonialCategory)}
                  ...
                  <div key={booking.id} className="bg-white rounded-xl shadow-md p-5 md:p-6 hover:shadow-lg transition-shadow">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                      <div className="rounded-lg p-4" style={{ backgroundColor: 'rgba(117, 90, 123, 0.05)' }}>
                        ...
                        <p className="font-semibold text-gray-800">{booking.offering?.name || 'Ceremonial package'}</p>
                        <p className="text-sm text-gray-600">{booking.offering?.category || 'Ceremonial package'}</p>
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => router.push('/dashboard/ceremonial/place-booking')} className="w-full rounded-lg px-4 py-3 font-medium text-gray-700 border-2 border-gray-300 hover:border-gray-400 transition-colors">
                      Open Calendar
                    </button>
                    <button onClick={() => router.push('/dashboard/ceremonial/posted-packages')} className="w-full rounded-lg px-4 py-3 font-medium text-white transition-colors" style={{ backgroundColor: '#755A7B' }}>
