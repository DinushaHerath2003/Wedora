...
export default function DraftPackagesPage() {
  ...
  const getCategoryBannerText = () => {
    switch (activeCategory) {
      case 'bridal-makeup':
        return 'Bridal Makeup Drafts';
      case 'hair-styling':
        return 'Hair Styling Drafts';
      case 'traditional-dressing':
        return 'Traditional Dressing Drafts';
      default:
      ...
      setPackages(
        offerings
          ...
          .map((offering) => {
            ...
            if (category === 'bridal-makeup' || category === 'bridal-makeup') {
              mappedCategory = 'bridal-makeup';
            } else if (category === 'hair-styling' || category === 'hair-styling') {
              mappedCategory = 'hair-styling';
            } else if (category === 'traditional-dressing' || category === 'traditional-dressing') {
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          ...
          <div className="mb-6">
            <div className="flex gap-2 md:gap-6 justify-center items-center overflow-x-auto pb-2">
              {['bridal-makeup', 'hair-styling', 'traditional-dressing'].map((category) => (
                <button key={category} onClick={() => setActiveCategory(category as BeautyCategory)} className="px-4 md:px-8 py-3 font-medium transition-all border-b-4 whitespace-nowrap" style={{ borderColor: activeCategory === category ? '#755A7B' : 'transparent', color: activeCategory === category ? '#755A7B' : '#999', fontWeight: activeCategory === category ? 'bold' : 'normal', background: activeCategory === category ? 'linear-gradient(to bottom, transparent, rgba(117, 90, 123, 0.05))' : 'transparent' }}>
                  {category === 'bridal-makeup' ? 'Bridal Makeup' : category === 'hair-styling' ? 'Hair Styling' : 'Traditional Dressing'}
                </button>
              ))}
            </div>
          </div>
              ...
              <div key={pkg.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group">
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={() => router.push(`/services/fashion-beauty/${user?.id || ''}`)} className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-emerald-500 font-semibold text-emerald-700 bg-white transition-all hover:bg-emerald-600 hover:text-white">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => router.push(`/dashboard/fashion-beauty?edit=${pkg.id}`)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg border-2 font-medium transition-all" style={{ borderColor: '#755A7B', color: '#755A7B', backgroundColor: 'white' }}>
                        <FaEdit size={16} /> Edit
                      </button>
                    </div>
          ...
          {filteredPackages.length === 0 && (
            <div className="text-center py-16">
              ...
              <button onClick={() => router.push('/dashboard/fashion-beauty')} className="px-6 py-3 rounded-lg font-medium text-white" style={{ backgroundColor: '#755A7B' }}>
