...
export default function AcceptBookingPage() {
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      const category = booking.offering?.category || 'bridal-makeup';
      const normalized = category === 'bridal-makeup' || category === 'bridal-makeup'
        ? 'bridal-makeup'
        : category === 'hair-styling' || category === 'hair-styling'
        ? 'hair-styling'
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="mb-6">
            ...
            <button onClick={() => router.push('/dashboard/fashion-beauty/overview')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100">
              <FaChartBar /> Overview
            </button>
          ...
          <div className="mb-6">
            <div className="flex gap-2 md:gap-6 justify-center items-center overflow-x-auto pb-2">
              {['bridal-makeup', 'hair-styling', 'traditional-dressing'].map((category) => (
                <button
                  key={category}
                  onClick={() => setActiveCategory(category as BeautyCategory)}
              ...
              <div className="xl:col-span-2 space-y-4">
                  ...
                  <div key={booking.id} className="bg-white rounded-xl shadow-md p-5 md:p-6 hover:shadow-lg transition-shadow">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                      <div className="rounded-lg p-4" style={{ backgroundColor: 'rgba(117, 90, 123, 0.05)' }}>
                        ...
                        <p className="font-semibold text-gray-800">{booking.offering?.name || 'Beauty package'}</p>
                        <p className="text-sm text-gray-600">{booking.offering?.category || 'beauty package'}</p>
                      </div>
                ...
                {filteredBookings.length === 0 && (
                  <div className="bg-white rounded-xl shadow-md p-10 text-center">
                    ...
                    <p className="text-gray-500">Requests for the selected beauty category will appear here.</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => router.push('/dashboard/fashion-beauty/place-booking')} className="w-full rounded-lg px-4 py-3 font-medium text-gray-700 border-2 border-gray-300 hover:border-gray-400 transition-colors">
                      Open Calendar
                    </button>
                    <button onClick={() => router.push('/dashboard/fashion-beauty/posted-packages')} className="w-full rounded-lg px-4 py-3 font-medium text-white transition-colors" style={{ backgroundColor: '#755A7B' }}>
