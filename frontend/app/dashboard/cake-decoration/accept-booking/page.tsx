...
import { FaBell, FaCalendarAlt, FaChartBar, FaCog, FaEdit, FaEye, FaFileInvoice, FaHeart, FaHome, FaMoon, FaPlus, FaTimes, FaCheck, FaHourglassHalf } from 'react-icons/fa';
...
interface BookingRow {
  ...
  offering?: { id: number; name: string; category: string; price: number } | null;
}
...
export default function CakeDecorationAcceptBookingPage() {
  ...
  const filteredBookings = useMemo(
    () => bookings.filter((booking) => normalizeCakeCategory(booking.offering?.category) === activeCategory),
  ...
  return (
    <div className="flex min-h-screen flex-col md:flex-row" style={{ backgroundColor: '#f5f5f7' }}>
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          ...
          <div className="mb-6">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1" style={{ backgroundColor: '#755A7B', color: 'white' }}><FaEye /> Accept Booking</button>
          </div>
          <div>
      ...
      <div className="flex-1 p-4 md:p-8">
        ...
        {loading ? (
          <div className="text-center py-16">Loading booking requests...</div>
        ) : (
            ...
            <div className="xl:col-span-2 space-y-4">
              {filteredBookings.map((booking) => (
                <div key={booking.id} className="bg-white rounded-xl shadow-md p-5">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2 mb-2"><FaHourglassHalf style={{ color: booking.status === 'pending' ? '#f59e0b' : '#10b981' }} /> <h3 className="text-lg font-bold text-gray-800">{booking.clientName || 'Guest Booking'}</h3></div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3 justify-end">
                    <button onClick={() => updateBookingStatus(booking.id, 'confirmed')} className="inline-flex items-center gap-2 rounded-lg px-4 py-3 font-medium text-white" style={{ backgroundColor: '#10b981' }}><FaCheck /> Approve</button>
                    <button onClick={() => updateBookingStatus(booking.id, 'cancelled')} className="inline-flex items-center gap-2 rounded-lg px-4 py-3 font-medium text-white" style={{ backgroundColor: '#ef4444' }}><FaTimes /> Reject</button>
                  </div>
                </div>
              ))}
              ...
              {filteredBookings.length === 0 && (
                <div className="bg-white rounded-xl shadow-md p-10 text-center">
                  ...
                  <p className="text-gray-500">Requests for the selected cake category will appear here.</p>
                </div>
              )}
            </div>
