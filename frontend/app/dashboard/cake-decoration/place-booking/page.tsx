...
export default function PlaceBookingPage() {
  ...
  const getCategoryBannerImage = () => {
    switch(activeCategory) {
      case 'wedding-cakes': return '/cake.png';
      case 'tiered-cakes': return '/pack1.png';
      case 'custom-designs': return '/pack2.png';
      default: return '/cake.png';
    }
  };

  const getCategoryBannerText = () => {
    switch(activeCategory) {
      case 'wedding-cakes': return 'Wedding Cakes Bookings';
      case 'tiered-cakes': return 'Tiered Cakes Bookings';
      case 'custom-designs': return 'Custom Designs Bookings';
      default: return 'Place Bookings';
    }
  };
    ...
    try {
      ...
      setBookings(
        data.map((booking) => ({
          ...
          category: normalizeCakeCategory(booking.offering?.category),
        })),
      );
    } catch (error) {
      console.error('Failed to load cake bookings', error);
  ...
  const fetchVendorOfferings = async (vendorId: number) => {
    try {
      ...
      console.error('Failed to load cake offerings', error);
      setApiError('Unable to load cake packages for booking.');
    }
  };
  ...
  const handleSaveBooking = async () => {
    ...
    const selectedOffering = offerings.find((offering) => normalizeCakeCategory(offering.category) === activeCategory) || offerings[0];
    if (!user || !Number.isFinite(vendorId) || vendorId <= 0) {
      setBookingError('Unable to save booking: invalid vendor session.');
      return;
    }
    if (!selectedOffering) {
      setBookingError('No cake package was found. Please add a package first.');
      return;
    }
    ...
    try {
      if (editingBooking) {
        ...
        setBookings((prev) => prev.map((booking) => booking.id === editingBooking.id ? {
          ...booking,
          clientName: newBooking.clientName,
          clientEmail: newBooking.clientEmail,
          clientPhone: newBooking.clientPhone,
      ...
      console.error('Failed to save cake booking', error);
  ...
  const handleDeleteBooking = async (id: string) => {
    ...
    try {
      ...
      console.error('Failed to delete cake booking', error);
    }
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="mb-6">
            ...
            <button onClick={() => router.push('/dashboard/cake-decoration/overview')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaChartBar /> Overview</button>
            <button onClick={() => router.push('/dashboard/cake-decoration')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaPlus /> Post Package</button>
            <button onClick={() => router.push('/dashboard/cake-decoration/posted-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaFileInvoice /> Posted Packages</button>
            <button onClick={() => router.push('/dashboard/cake-decoration/draft-packages')} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1 transition-colors text-gray-600 hover:bg-gray-100"><FaEdit /> Draft Package</button>
          </div>
      ...
      <div className="flex-1 flex flex-col">
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="mb-6 md:mb-8 rounded-lg overflow-hidden shadow-lg" style={{backgroundImage: `url(${getCategoryBannerImage()})`, backgroundSize: 'cover', backgroundPosition: 'center', height: '200px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: '2px solid rgba(117, 90, 123, 0.2)'}}>
            ...
            <p className="text-sm md:text-xl text-white" style={{textShadow: '2px 2px 4px rgba(0,0,0,0.7)'}}>Schedule cake consultations and manage appointments</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            ...
            <div className="space-y-3">
              ...
              {bookings.filter((booking) => booking.category === activeCategory).length === 0 && (
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <textarea value={newBooking.description} onChange={(e) => setNewBooking({...newBooking, description: e.target.value})} className="w-full px-4 py-3 border border-gray-200 rounded-lg text-gray-900" rows={4} placeholder="Cake tasting, delivery, setup, or design notes" />
              </div>
