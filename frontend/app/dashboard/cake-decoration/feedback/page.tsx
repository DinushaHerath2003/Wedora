...
interface VendorReview {
  ...
  offering?: { id: number; name: string; category: string; price: number; images?: string[] } | null;
}

export default function CakeDecorationFeedbackPage() {
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      try {
        ...
        console.error('Failed to load cake feedback', error);
  ...
  const groupedByPackage = useMemo(() => reviews.reduce<Record<string, VendorReview[]>>((groups, review) => {
      ...
      <aside className="w-full md:w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 p-4">
          ...
          <div>
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-1" style={{ backgroundColor: '#755A7B', color: 'white' }}><FaHeart /> Feedback</button>
      ...
      <div className="flex-1 p-4 md:p-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
          <section className="bg-white rounded-xl shadow-md p-6">
            ...
            <div className="flex gap-1 mb-2">{[...Array(5)].map((_, index) => <FaStar key={index} style={{ color: index < Math.round(averageRating) ? '#fbbf24' : '#d1d5db' }} />)}</div>
              ...
              {ratingCounts.map((item) => {
                ...
                return (
                  <div key={item.rating} className="flex items-center gap-3">
                    ...
                    <div className="h-3 flex-1 rounded-full bg-gray-100 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${percent}%`, backgroundColor: '#755A7B' }} /></div>
        ...
        {loading ? (
            ...
            {Object.entries(groupedByPackage).map(([packageName, packageReviews]) => (
              <section key={packageName} className="bg-white rounded-xl shadow-md p-6">
                <div className="flex items-center justify-between gap-3 mb-5">
                  ...
                  <button onClick={() => router.push(`/dashboard/cake-decoration/posted-packages/${packageReviews[0].offeringId}`)} className="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 font-medium text-white" style={{ backgroundColor: '#755A7B' }}><FaEye /> View Package</button>
                </div>
                  ...
                  {packageReviews.map((review) => (
                    <article key={review.id} className="rounded-xl border border-gray-200 p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <div className="flex gap-1">{[...Array(5)].map((_, index) => <FaStar key={index} size={15} style={{ color: index < review.rating ? '#fbbf24' : '#d1d5db' }} />)}</div>
                      </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-12 text-center">
            ...
            <p className="text-gray-500">Customer reviews for your cake packages will appear here.</p>
          </div>
        )}
      </div>
    </div>
